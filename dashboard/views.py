...
def index(request):
    ...
    student_name = Student(last_name='naghibi').last_name
    course_name = Course(name='CPSC 317').name
    #assessment = Assessment.get_assessment()
    homework_avg = 88#Grades.objects.filter(assessment_type__exact='h').avg()
