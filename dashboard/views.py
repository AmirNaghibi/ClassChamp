...
def index(request):
    ...
    hw_weight = 0 if (Grades.objects.all().filter(assessment_type='h').count()==0) else Assessment.objects.all().get(course_name='cpsc 317').homeworks
    qz_weight = 0 if (Grades.objects.all().filter(assessment_type='q').count()==0) else Assessment.objects.all().get(course_name='cpsc 317').quizzes
    mt_weight = 0 if (Grades.objects.all().filter(assessment_type='m').count()==0) else Assessment.objects.all().get(course_name='cpsc 317').midterms
    fn_weight = 0 if (Grades.objects.all().filter(assessment_type='f').count()==0) else Assessment.objects.all().get(course_name='cpsc 317').final
